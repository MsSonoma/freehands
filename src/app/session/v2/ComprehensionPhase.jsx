...
export class ComprehensionPhase {
  ...
  async start(options = {}) {
    ...
    // Start play timer (exploration time for opening actions)
  ...
  async go() {
    ...
    // Without this, resume can incorrectly land back on Opening Actions (timerMode still 'play' from init).
    this.#emit('requestSnapshotSave', {
      trigger: 'comprehension-go',
      data: {
        nextQuestionIndex: 0,
        score: this.#score,
    ...
    try {
      ...
      this.#audioEngine.playAudio(introAudio || '', [intro]).catch((err) => {
        console.error('[ComprehensionPhase] Intro playback error:', err);
        finishIntro();
      });
    } catch (err) {
