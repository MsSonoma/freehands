...
test('instructional execution ownership keeps historical end reasons but no longer auto-expires a foreign browser', () => {
  ...
  assert.ok(continuitySql.includes('v_resume_from public.lesson_sessions%rowtype'))
  assert.match(continuitySql, /v_resume_found/)
  assert.ok(continuitySql.includes("s.ended_reason in ('expired', 'moved', 'released', 'taken_over')"))
  assert.ok(continuitySql.includes("'resume_after_' || coalesce(v_resume_from.ended_reason, 'interruption')"))
  assert.match(continuitySql, /snapshotHandoffId/)
  assert.ok(continuitySql.includes('h.target_execution_session_id = v_requested_active.id'))
  assert.ok(continuitySql.includes("h.state in ('pending', 'source_ready')"))
