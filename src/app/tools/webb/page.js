...
export default function WebbVoiceStudioPage() {
  const [text, setText] = useState('')
  const [status, setStatus] = useState('idle')
  const [audioUrl, setAudioUrl] = useState(null)
  const [filename, setFilename] = useState('webb-voice.mp3')
  ...
  function handleDownload() {
    ...
    a.href = url
    a.download = filename || 'webb-voice.mp3'
    a.click()
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 720 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 6, color: '#fff' }}>
          Mrs. Webb — Voice Studio
        </h1>
        <p style={{ fontSize: 13, color: '#888', marginBottom: 32 }}>
          Internal tool · en-US-Neural2-F · {'\u00A0'}
        ...
        <textarea
          ...
          placeholder="Enter the text you want Mrs. Webb to read…"
          rows={10}
          style={{
            width: '100%',
            background: '#1a1a1a',
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
          <button
            ...
            style={{
              background: status === 'loading' ? '#333' : '#0f766e',
              color: '#fff',
              border: 'none',
              borderRadius: 8,
              padding: '10px 24px',
