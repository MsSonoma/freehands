...
test('Generator preserves draft and approval association state while learner active-Syllabus library behavior stays separate', () => {
  ...
  assert.match(generator, /body: JSON\.stringify\(\{ file, learnerId: intendedLearnerId \}\)/)
  assert.match(generator, /json\?\.association\?\.learner_id !== intendedLearnerId/)
  const approval = fs.readFileSync(path.resolve('src/app/api/facilitator/lessons/approve/route.js'), 'utf8')
  assert.match(approval, /await requireAssociationLearner/)
  assert.match(approval, /readinessState: 'approved'/)
  assert.match(approval, /subject: confirmedLesson\.subject, title: confirmedLesson\.title/)
...
test('SyllabusDocument keeps rows readable while facilitator controls live in the lesson overlay', () => {
  ...
  assert.match(document, /role=\{onSelectLesson \? 'button' : undefined\}/)
  assert.doesNotMatch(document, /className=\{styles\.lessonAction\}/)
  assert.doesNotMatch(document, /Assigned teacher[\s\S]*<select/)
  assert.match(overlay, /Assigned teacher<select/)
  ...
  assert.match(overlay, /facilitator_attested_webb_completion_v1_import/)
...
test('scheduled and on-demand Slate launches preserve exact occurrence and distinct purpose behavior', () => {
  ...
  assert.match(home, /syllabusItem\?\.practice_occurrence_id \|\| syllabusItem\?\.occurrence_id/)
  assert.match(home, /const runPurpose = syllabusItem\?\.run_purpose \|\| 'practice'/)
  assert.match(home, /purpose=\$\{encodeURIComponent\(runPurpose\)\}/)
  assert.match(home, /purpose=practice/)
