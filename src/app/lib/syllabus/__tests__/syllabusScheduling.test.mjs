...
test('Syllabus placement uses the native dialog, canonical route, PIN retry, and authoritative reload', () => {
  ...
  const actionHrefStart = document.indexOf('const actionHref')
  const actionHrefSource = document.slice(actionHrefStart, document.indexOf('return (', actionHrefStart))
  ...
  assert.match(saveSource, /fetch\('\/api\/lesson-schedule'/)
  assert.match(saveSource, /postLessonScheduleWithCapacityPin/)
  assert.match(saveSource, /await loadCurrent\(requestLearnerId\)/)
  assert.doesNotMatch(saveSource, /setSyllabus|setLearningProposal|timeline_items/)
  assert.match(page, /item\?\.is_explicit_schedule === true/)
  assert.match(page, /item\?\.placement_kind === 'scheduled'/)
  assert.match(document, /Use existing lesson/)
  assert.match(page, /existingLessonKey/)
  assert.match(page, /forecastLineageId: scheduleDialog\.item\?\.forecast_lineage_id \|\| scheduleDialog\.item\?\.lineage_id/)
})
