...
test('Syllabus placement uses the native dialog, canonical route, PIN retry, and authoritative reload', () => {
  ...
  assert.ok(detailOverlay.includes("localExplicitSchedule ? 'Reschedule' : 'Schedule'"))
  assert.ok(detailOverlay.includes('setLocalExplicitSchedule(true)'))
  assert.ok(detailOverlay.includes('postLessonScheduleWithCapacityPin'))
