...
test('Syllabus UX preserves week position and hardens overlays and async planning races', () => {
  ...
  assert.ok(detailOverlay.includes('Replacing...'))
...
test('inactive forecast presentation stays readable in the weekly document and keeps its actions in the detail overlay', () => {
  ...
  assert.ok(facilitator.includes('proposalRevision={learningProposal?.proposal_revision || null}'))
  assert.match(document, /Suggested/)
  ...
  assert.doesNotMatch(document, /suggestionAction/)
  for (const action of ['Edit concept', 'Replace suggestion', 'Generate lesson', 'Use existing lesson']) assert.match(detailOverlay, new RegExp(action))
  assert.match(document, /Use this forecast/)
