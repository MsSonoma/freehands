...
test('Syllabus UX preserves week position and hardens overlays and async planning races', () => {
  ...
  const document = fs.readFileSync(new URL('../../../components/syllabus/SyllabusDocument.js', import.meta.url), 'utf8')
  const workspace = fs.readFileSync(new URL('../../../components/syllabus/SyllabusPlanningWorkspace.js', import.meta.url), 'utf8')
  assert.match(document, /\[learnerId, restoreWeekStart, today\]/)
  ...
  assert.match(document, /Replacing…/)
  assert.match(facilitator, /event\.key !== 'Escape'/)
  assert.match(workspace, /event\.key === 'Escape'/)
  assert.match(workspace, /if \(result\) setEditor\(null\)/)
  assert.match(workspace, /role="alert"/)
  assert.doesNotMatch(workspace, /role="dialog"|aria-modal|styles\.backdrop|document\.body\.style\.overflow/)
})

test('inactive forecast presentation lives inside the weekly document with complete existing actions', () => {
  const facilitator = fs.readFileSync(new URL('../../../facilitator/syllabus/page.js', import.meta.url), 'utf8')
  const document = fs.readFileSync(new URL('../../../components/syllabus/SyllabusDocument.js', import.meta.url), 'utf8')
  assert.doesNotMatch(facilitator, /className=\{styles\.learningProposal\}|Proposed forecast|Next week&apos;s open Syllabus slots/)
  assert.match(facilitator, /proposedForecastItems=\{learningProposal\?\.forecast_items \|\| \[\]\}/)
  assert.match(facilitator, /proposalRevision=\{learningProposal\?\.proposal_revision \|\| null\}/)
  assert.match(document, /Suggested · not active/)
  for (const action of ['Edit', 'Replace', 'Generate lesson', 'Use this forecast']) assert.match(document, new RegExp(action))
  assert.match(document, /data-forecast-lineage=\{item\.lineage_id\}/)
  assert.match(document, /projectLearningForecastForWeek/)
  assert.match(document, /syllabusDayPresentation/)
})

test('forecast progress, failure, retry, and empty state are contained in the exact target week', () => {
  const facilitator = fs.readFileSync(new URL('../../../facilitator/syllabus/page.js', import.meta.url), 'utf8')
  const document = fs.readFileSync(new URL('../../../components/syllabus/SyllabusDocument.js', import.meta.url), 'utf8')
  assert.match(document, /week\.week_start === startOfSyllabusWeek\(proposedForecastTargetWeek\)/)
  assert.match(document, /Preparing suggestions for this week/)
  assert.match(document, /No new suggestions are needed for this week/)
  assert.match(document, /Retry forecast/)
