...
export class MasteryEvidenceClient {
  ...
  recordAskUsed({
    ...
    const suffix = [
      ...
      askMode,
      interactionId || 'no-interaction',
      Number.isFinite(Number(turnIndex)) ? `turn-${Number(turnIndex)}` : this.eventSequence + 1,
    ].join(':');
    return this.#trackWrite(this.#recordEvent({
      ...
      payload: {
        ...
        turn_index: Number.isFinite(Number(turnIndex)) ? Number(turnIndex) : null,
        input_mode: inputMode || null,
        strategy_used: strategyUsed || null,
        target_type: targetType || null,
        target_text: targetText == null ? null : String(targetText),
        target_term: targetTerm == null ? null : String(targetTerm),
      },
    }));
  }

  recordComprehensionSignal({
    phase,
    itemId = null,
    itemPurpose = null,
    itemExposureId = null,
    ...
    return this.#trackWrite(this.#recordEvent({
      eventType: STAGE_2_EVIDENCE_EVENT_TYPES.COMPREHENSION_SIGNAL,
      occurredAt: this.now(),
      phase,
      suffix,
      itemId: itemId || legacyItemFingerprint || null,
      ...
      assessmentRole,
      evidencePurpose,
      result: diagnostic && typeof diagnostic === 'object' ? diagnostic : null,
      payload: {
        source,
        legacy_item_fingerprint: legacyItemFingerprint || null,
        interaction_id: interactionId || null,
        ...
        learner_message: learnerMessage == null ? null : String(learnerMessage),
        observed_signal: observedSignal || null,
        target_type: targetType || null,
        target_text: targetText == null ? null : String(targetText),
        target_term: targetTerm == null ? null : String(targetTerm),
        target_definition: targetDefinition == null ? null : String(targetDefinition),
