...
test('Daily and Weekly runs route into Slate while protected APIs remain authoritative', () => {
  ...
  const legacyLearn = fs.readFileSync(path.resolve('src/app/learn/lessons/page.js'), 'utf8')
  ...
  assert.match(learn, /followUpCards\.map/)
  assert.match(legacyLearn, /redirect\(['"]\/learn['"]\)/)
  assert.doesNotMatch(legacyLearn, /followUpCards\.map|reviewRunId|getFollowUpRun|actOnFollowUp|useEffect/)
