2026-03-23 | Fix: YouTube video black screen on iPad (iOS Safari sandbox). Root cause: `sandbox="allow-scripts allow-same-origin allow-presentation"` was too restrictive for iPadOS Safari. YouTube's IFrame API player on iOS internally uses popup windows and form submissions during bootstrap — without `allow-popups` and `allow-forms` the player loads the DOM but renders a black frame. Separately, `allow="...picture-in-picture"` was missing `fullscreen` as a Feature Policy token, which older WebKit builds require separately from the `allowFullScreen` attribute. Fix: updated iframe sandbox to `allow-scripts allow-same-origin allow-presentation allow-popups allow-popups-to-escape-sandbox allow-forms`; updated `allow` to include `fullscreen`. Build: ✓. Recon prompt: "youtube iframe black screen iPad iOS sandbox allow".
