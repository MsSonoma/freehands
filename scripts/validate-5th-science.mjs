// Validate all 5th grade science lesson JSONs for required shape
...
const lessonsDir = toAbsolute('public/lessons/science');
...
async function main() {
  ...
  if (!files.length) {
    console.error(`No 5th grade science files found under ${lessonsDir}`);
    process.exit(1);
  }
  ...
  console.log(`\nValidated ${total}/${files.length} 5th grade science files.`);
  if (allErrors.length) {
    process.exit(1);
  }
}
