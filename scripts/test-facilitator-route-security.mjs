...
test('lesson library auto-loads and keeps multi-learner choice explicit', () => {
  ...
  assert.match(source, /data-primary-action/)
  assert.match(source, /data-more-actions/)
  assert.match(source, />\s*Edit\s*<\/button>/)
  assert.match(source, /noteText \? 'Edit note' : 'Notes'/)
  assert.match(source, />\s*Detailed history\s*<\/button>/)
  assert.match(source, /In progress since \{formatDateTime\(inProgressAt\)\}/)
  assert.match(source, /Last completed \{formatDateOnly\(lastCompletedAt\)\}/)
  assert.match(source, /Medal: \{emojiForTier\(medalInfo\.medalTier\)\}/)
  assert.match(source, /Golden Key active/)
})
