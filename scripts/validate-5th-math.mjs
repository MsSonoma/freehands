// Validate all 5th grade math lesson JSONs for required shape
// - Arrays for: sample, truefalse, multiplechoice, fillintheblank, shortanswer
// - wordProblems: array if present
...
const lessonsDir = toAbsolute('public/lessons/math');
...
async function main() {
  ...
  if (!files.length) {
    console.error(`No 5th grade math files found under ${lessonsDir}`);
    process.exit(1);
  }
  ...
  for (const file of files) {
    ...
    if ('wordProblems' in data) {
      assert(isArrayWithMin(data.wordProblems, 10), `${file} key 'wordProblems' must be an array with >= 10 items`, errs);
    }
  ...
  console.log(`\nValidated ${total}/${files.length} 5th grade math files.`);
  if (allErrors.length) {
    process.exit(1);
  }
}
