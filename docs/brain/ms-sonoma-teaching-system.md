...
- Games opens a full-screen GamesOverlay (memory, snake, catch, maze, whack-a-mole, platform jumper) with the play timer badge from SessionTimer and a grade selector (grade drives game difficulty); closes via X/background click or automatically when leaving play time or when play timer expires.
