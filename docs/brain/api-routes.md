...
- **Transcript boundary**: Server code derives the authorized storage location and returns a short-lived signed URL. Exact session-scoped artifacts are preferred. A legacy consolidated transcript is eligible only when its single ledger segment timestamp-matches the authorized canonical session; multi-session consolidated history fails closed. Raw storage paths are not response authority, and a missing transcript does not erase session/evidence history
