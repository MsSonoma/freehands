...
The Syllabus instructional forecast may consume a bounded projection of authorized `facilitator-evidence-v1` reports to choose instructional progression and pacing. `reporting.js` remains the only mastery interpretation authority and now emits `learning_summary` as a deterministic rendering of its already-authoritative fields: headline and narrative copy the independent-evidence label and detail, unresolved text copies an unavailable/not-yet-measured retention state, planning meaning copies the first existing option label, and assistance counts only already-classified hint/retry events. The summary creates no state, classification, score, or mastery interpretation. Review History renders that summary as its sole prominent conclusion source; missing older summaries receive neutral unavailable copy rather than a conclusion reconstructed from raw facets. Assistance counts remain subordinate and are omitted when zero, while all four facets remain available only in Evidence details. Completed occurrence-bound Slate cards use their own report `learning_summary`; scheduled Slate intent is not evidence. The forecast receives at most 12 whitelisted report projections and never receives raw events, transcript text, medals, or scores. Missing older summaries fail soft. The evidence-informed `learning_forecast` is the sole current instructional forecast; the former `mastery_reforecast` creation, read-model, UI, and activation authority is retired, with recognition retained only for readable historical items. This does not make instructional forecasting a mastery engine. Slate retains Daily Follow-Up, Weekly Review, recovery, retention, and mastery-check authority; Stage 8 options are not automatically converted into new instructional lessons.
...
For learners with an active Syllabus, enabled Daily and Weekly review policy is projected into the Syllabus as `item_type: review` rows. These rows are not lessons, assignments, notifications, or curriculum objects. They do not increment lesson quotas, reopen a completed lesson, create lesson snapshots, change scores/medals, or mutate the Stage 1-8 session evidence rows. Review Settings remain facilitator-controlled per learner under the existing mastery-evidence feature flag. After a review is completed, the full review row is not kept as a large Syllabus card: completed reviews project onto their actual local completion date as one compact Review history card per day. Multiple completed reviews on the same day share that card, and opening it shows the grouped review types, administering teacher when recorded, and the lesson checklist each review covered. Mr. Slate supplemental completion history remains a separate compact `Mr. Slate` history card so a Sonoma- or Webb-administered review is never attributed to Slate. Server-verified pre-evidence `slate_drill_completion` history is projected into the Slate history card at read time, using the recorded local completion date and lesson metadata; this is a display backfill only and does not rewrite the legacy evidence rows. Current Mr. Slate drill completion is also written server-side to `slate_session_completions` at the point the drill goal is reached. Those rows are supplemental completion facts only: they carry learner, lesson, occurrence, run purpose, and timestamps for Syllabus history, but do not grant instructional completion or mastery.

The protocols and pools remain explicit:

- `daily-review-v1` is the same-day Syllabus review protocol. One Daily Review row represents one learning day, shows each constituent lesson as complete or incomplete, and becomes startable only after every lesson in that day is complete. Missed Daily Reviews do not stack. The protocol uses a dedicated `dailyReview` reserved pool when present and can consume an unexposed legacy `dailyFollowup` reserved pool for older lessons. Quiz length scales with the contributing lessons: each unique lesson may contribute up to four valid unexposed reserved review questions, subject to a 32-item safety ceiling. Its outcomes use demonstration/review language, never delayed-retention claims.
- `daily-followup-v1` remains the historical delayed protocol. It requires at least 24 hours after one eligible Stage 6 success and keeps its original `retained` / `needs_review` meaning. Existing runs and evidence are not relabeled.
- `weekly-review-v1` uses only `weeklyReview` aliases. It selects eligible recent lesson targets from the seven-local-day learning window ending at the configured weekday activation, deduplicates by contributing lesson, and may take up to four valid unexposed Weekly Review questions from each lesson, subject to a 32-item safety ceiling. The current weekly activation remains available until the next activation; missed weeks do not stack. The Syllabus projects that review on the configured weekday and shows lesson-completion progress leading into it.
- Completing a same-day Daily Review or historical Daily Follow-Up does not consume Weekly eligibility. Weekly result facts can record earlier Daily retrieval context for the same Stage 6 anchor.
...
The first learner response is immutable through an idempotency key and database uniqueness. Resume uses the same run and selected item. Verbatim Repeat is recorded but remains non-disqualifying. Answer reveal is recorded before response and prevents an independent interpretation. Refresh, Start Over, and lesson-session state cannot reset a review run because review persistence is separate from snapshots and lesson local storage. The facilitator may choose Ms. Sonoma, Mrs. Webb, or Mr. Slate to administer a Daily/Weekly review quiz. That teacher choice is stored in run metadata and controls the quiz presentation, character video, and TTS voice only; it does not change the reserved item pool, qualification, scoring, evidence meaning, or convert the review into a lesson. The quiz requires a learner Begin Quiz gesture before presentation so media is unlocked before the first spoken question.
...
- Do not bind a Stage 8 report, Slate activity, or Daily/Weekly review to a repeated Syllabus lesson by title, lesson key, or date. Contextual Review History first proves the exact composed occurrence; instructional evidence then joins through canonical `lesson_sessions.id`, and review data requires an authorized mastery-check anchor.
...
- Do not treat Daily or Weekly review rows/runs as lessons, count them against lesson quotas, create snapshots for them, or auto-assign/reopen curriculum.
