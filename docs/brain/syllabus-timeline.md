...
When a planned occurrence becomes canonical actual history, an already-open facilitator lesson overlay promotes to that exact `actual:<lesson_sessions.id>` representation instead of retaining stale planned readiness. Completed presentation uses the immutable actual teacher and terminal state, clears stale progress/availability and manual historical-record controls, protects the actual occurrence from removal, and shows separate Scheduled and Completed timestamps when the source intent date is provable. Retired source dates are loaded only by exact server-owned occurrence IDs and are presentation provenance; they never reactivate old revisions or planning intent. The completed lesson's Lesson record control links directly into the same occurrence-scoped transcript/history overlay.
...
Instructional transcript resolution prefers an exact session-scoped artifact. Mrs. Webb now writes transcript autosaves under the canonical execution-session identity. For older Webb records that predate that path, the server may use the consolidated lesson transcript only when its ledger contains exactly one segment and that segment's start/end timestamps match the authorized canonical session within the bounded tolerance; multi-session consolidated ledgers fail closed rather than crossing occurrence boundaries.
